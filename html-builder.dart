    ...
    return ([Map<String, String> attributes = const {}]) {
        return ([dynamic content]) {
            ...
            final body = content != null 
                ...
                : _escapeContent(content.toString()) 
                : '';
...
String html(dynamic content) {
    final body = content is Iterable ? content.join() : content.toString();
    return '<!DOCTYPE html><html>${_escapeContent(body)}</html>';
}

String head(dynamic content) {
    final body = content is Iterable ? content.join() : content.toString();
    return '<head>${_escapeContent(body)}</head>';
}

String body(dynamic content) {
    final body = content is Iterable ? content.join() : content.toString();
    return '<body>${_escapeContent(body)}</body>';
}
...
TagBuilderWithAttrs _createTag(String tagName, {bool selfClosing = false}) {
  return ([Map<String, String> attributes = const {}]) {
    return ([dynamic content]) {
      ...
      // Handle regular tags with content
      final body = content != null ? content.toString() : ''; // Removed _escapeContent here
      return '<$tagName$attrs>$body</$tagName>';
    };
  };
}
